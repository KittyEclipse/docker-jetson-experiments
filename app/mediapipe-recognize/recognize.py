...
def run(model: str, num_hands: int,
  ...
  cap = cv2.VideoCapture(gstreamer_pipeline(capture_width=width, capture_height=height), cv2.CAP_GSTREAMER) #cv2.VideoCapture(camera_id)
  ...
  while cap.isOpened():
    ...
    if recognition_frame is not None:
        ...
        GPIO.output(LED_PIN, GPIO.HIGH if power_led else GPIO.LOW)
